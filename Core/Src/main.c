...
int main(void)
{
  ...
  while (1) {
    ...
    //     OLED_ShowString(1,1,"TIMEOUT! I2S RX flag never set");
    // } else if (status == HAL_ERROR) {
    //     OLED_ShowString(1,1,"ERROR! Code=, State=");
    // } else if (status == HAL_BUSY) {
    //     OLED_ShowString(1,1,"BUSY! I2S is locked");
    // }
    // HAL_Delay(200);

    // uint32_t rx_data = 0;
    // // 尝试读取一个 I2S 数据帧
    // if (HAL_I2S_Receive(&hi2s3, (uint16_t*)&rx_data, 1, 100) == HAL_OK) {
    //     // 如果读到非零数据，说明 I2S 通信已建立
    //     OLED_ShowNum(1,1,rx_data,10); 
    // } else {
    //     OLED_ShowString(1,1,"I2S RX Timeout/Error!\n");
    // }
    // HAL_Delay(100);
